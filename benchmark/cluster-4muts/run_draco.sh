...
for T in $(seq 0 $(( NTASK - 1 ))); do
    ...
    if [[ -v TASK ]]; then
        ...
        if [[ ! -f $BAM ]]; then
            ...
            rm -rf $OUT_MAP
            rf-map -p 1 --working-threads $CPUS --bowtie2 --bowtie-softclip -t $TMP/rf_map -o $OUT_MAP --bowtie-index $INDEX $FQ12
            rm $FQ12 ${INDEX}.*.bt2
        fi
        ...
        if [[ ! -f $MM ]]; then
            ...
            if grep -q "^$REF" $ALL_MASKS; then
                ...
                rf-count -p 1 --working-threads $CPUS --fast -m --mutation-map -o $OUT_COUNT -f $REFS_DIR/$REF_FILE $BAM --mask-file $REF_MASK
                rm $REF_MASK
            else
                # This reference does not have primers.
                rf-count -p 1 --working-threads $CPUS --fast -m --mutation-map -o $OUT_COUNT -f $REFS_DIR/$REF_FILE $BAM
            fi
        fi
        ...
        if [[ ! -f $JSON ]]; then
            ...
            JSON_TMP=${JSON}.tmp
            rm -f $JSON_TMP
            draco --processors $CPUS --allNonInformativeToOne --mm $MM --output $JSON_TMP
            mv $JSON_TMP $JSON
        fi
        ...
        if [[ ! -f $RC_DRACO ]]; then
            rm -rf $OUT_JSON2RC
            rf-json2rc --median-pre-cov 1 --median-cov 1 --min-confs 1 -j $JSON -r $RC_COUNT -o $OUT_JSON2RC
        fi
        ...
        if compgen -G "$OUT_NORM/*.xml" > /dev/null; then
            ...
            rm -rf $OUT_NORM
            rf-norm -p $CPUS --scoring-method 4 --raw --reactive-bases AC -D 6 -t $RC_DRACO -o $OUT_NORM
        fi
    else
