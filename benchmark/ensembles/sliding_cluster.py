...
from seismicrna.cluster.emk import calc_mean_arcsine_distance_clusters
from seismicrna.cluster.report import ClusterReport
from seismicrna.core import path
from seismicrna.core.arg import (CMD_JOIN,
                                 merge_params,
                                 extra_defaults,
                                 opt_join_clusts,
                                 opt_tile_length,
                                 ...
                                 opt_verbose)
from seismicrna.core.dataset import MutsDataset
from seismicrna.core.error import (IncompatibleValuesError,
                                   InconsistentValueError,
                                   OutOfBoundsError)
from seismicrna.core.logs import logger, set_config
from seismicrna.core.rel import RelPattern
from seismicrna.core.report import KsWrittenF, End5F, End3F
from seismicrna.core.run import run_func
from seismicrna.core.seq import DNA, DuplicateReferenceNameError
from seismicrna.core.task import dispatch
from seismicrna.join import joined_mask_report_exists, join_regions
from seismicrna.mask.main import load_regions
from seismicrna.mask.report import MaskReport
from seismicrna.mask.write import get_pattern
...
def calc_ref_tile_length(datasets: Iterable[MutsDataset],
                           pattern: RelPattern,
                           mask_discontig: bool,
                           min_mut_gap: int):
    ...
    tile_length = max(int(np.ceil(2. * total_cover / total_muts)), 1)
    logger.routine(f"Ended calculating optimal region length: {tile_length}")
    return tile_length
    ...
    for ref, ref_total_regions in total_regions.dict.items():
        ...
        if tile_length > 0:
            ...
            try:
                ...
                ref_tile_length = calc_ref_tile_length(datasets[ref],
                                                           pattern,
                                                           mask_discontig,
                                                           min_mut_gap)
...
CMD_SLIDING = "sliding"
@run_func(CMD_SLIDING, extra_defaults=extra_defaults)
...
@command(CMD_SLIDING, params=params)
def cli(*args, **kwargs):
    """ Infer independent structure ensembles along an entire RNA. """
