...
DREEM_OUT=$PWD/out-dreem-redhat
mkdir -p $DREEM_OUT 
...
REFS_DIR=$SIM/refs
REF_FILES=( $(ls $REFS_DIR/rna-280-2-*.fa) )
NSAMP=${#SAMPLE_DIRS[@]}
NREF=${#REF_FILES[@]}
NTASK=$(( NSAMP * NREF ))
...
for T in $(seq 0 $(( NTASK - 1 ))); do
    ...
    TIME_FILE=$DREEM_OUT/time_dreem_${SAMPLE_REF}.txt
    ...
    if [[ -v TASK ]]; then
        ...
        if [[ ! -f $TIME_FILE ]]; then
            ...
            cd $DREEM_CODE
            /usr/bin/time -f "%e" -o $TIME_FILE python $DREEM_PY $TMP $OUT $SAMPLE $REF $END5 $END3 --fastq
            cd -
            rm -r $TMP
        fi
    else
